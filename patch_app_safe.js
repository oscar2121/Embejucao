...
// 2. Inject Button
const searchBtn = `<TouchableOpacity
...
const newBtn = `<View style={{ flexDirection: 'row', gap: 8 }}>
              <TouchableOpacity
...
const replaceMarker = `{/* ─── MODAL FIAR PEDIDO ─── */}`;
if (c.includes(replaceMarker)) {
  c = c.replace(replaceMarker, modalStr + "\n      " + replaceMarker);
} else {
  // Try to insert at the end of the App component by finding the last return statement of App
  const endMarker = `    </SafeAreaView>\n  );\n}\n// --- VISTA PEDIDO ---`;
  if (c.includes(endMarker)) {
    c = c.replace(endMarker, modalStr + "\n" + endMarker);
  }
}
